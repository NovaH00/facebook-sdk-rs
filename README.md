...
The SDK uploads each photo or video as an unpublished media item first (`POST /me/photos` or `POST /me/videos`), then attaches them to the feed post.
...
| Field | Type |
|-------|------|
| `id` | `String` — the ID of the newly created post or video |
| `post_url` | `String` — the canonical public URL to view the post on Facebook |
